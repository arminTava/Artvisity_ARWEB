...
export default class ARnft {
    ...
    async _initialize(markerUrls, names, stats) {
        ...
        getConfig(this.configUrl)
            ...
            btnDiv.id = "stats";
            btnDiv.className = "ui stats";
            let btn = document.createElement("button");
            btn.className = "stats1";
            btn.innerHTML = "Back";
            btn.onclick = function () {
                var btnBacKEvent = new Event("btnBackEvent");
                document.dispatchEvent(btnBacKEvent);
            };
            btnDiv.appendChild(btn);
            const loading = document.getElementById("loading");
            document.body.insertBefore(btnDiv, loading);
